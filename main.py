...
from textual.widgets import Header, Footer, DirectoryTree, TextArea, RichLog, Label, Input, TabbedContent, TabPane, Static, Button, Tree, Switch, DataTable, ContentSwitcher, Select, OptionList
...
class HorizontalResizer(Static):
    ...
    def on_mouse_move(self, event: events.MouseMove) -> None:
        if self._dragging:
            try:
                target = self.app.query_one(f"#{self.target_id}")
                new_height = self.app.console.size.height - event.screen_y - self.drag_offset
        ...
        with Horizontal(id="sql-body"):
            ...
            with Vertical(id="sql-editor-panel"):
                yield TextArea(
                    ...
                    theme=getattr(self.app, "settings", {}).get("editor_theme", "monokai")
                )
    ...
    def run_query(self) -> None:
        ...
        try:
            ...
            if cur.description:
                ...
                conn.commit()
                status.update(f"✅ Query OK · {cur.rowcount} row(s) affected · {elapsed:.1f}ms")
                self._load_schema()
        ...
        try:
            ...
            with open(out_path, "w", newline="", encoding="utf-8") as f:
                ...
                for row_key in results.rows.keys():
                    writer.writerow([str(results.get_cell(row_key, col)) for col in col_keys])
            self.query_one("#sql-status", Label).update(f"✅ Exported → {out_path.name}")
        except Exception as e:
            self.query_one("#sql-status", Label).update(f"[red]Export failed: {e}[/red]")
...
class CommandPaletteModal(ModalScreen):
    ...
    COMMANDS = [
        ...
        ("\ud83d�\ufe0f  Global Replace",      "global_replace"),
    ...
    def compose(self) -> ComposeResult:
        with Vertical(id="cp-dialog"):
            yield Input(placeholder="\u2315 Type a command or filename...", id="cp-input")
...
class UnsavedChangesModal(ModalScreen):
    ...
    def compose(self) -> ComposeResult:
        with Vertical(id="unsaved-dialog"):
            yield Label(f"\u26a0\ufe0f  {self.count} unsaved file(s)", id="unsaved-title")
            yield Label("You have unsaved changes. What would you like to do?")
            with Horizontal(id="unsaved-buttons"):
                yield Button("\ud83d� Save All & Quit", id="btn-save-quit", variant="success")
    ...
    def save_quit(self): self.dismiss("save")
    @on(Button.Pressed, "#btn-force-quit")
    def force_quit(self): self.dismiss("force")
    ...
    def compose(self) -> ComposeResult:
        with Vertical(id="replace-dialog"):
            ...
            with Horizontal():
                ...
                yield Button("Refactor All Files", id="btn-run-replace", variant="success")
    ...
    def compose(self) -> ComposeResult:
        with Vertical(id="trust-dialog"):
            ...
            with Horizontal(id="trust-buttons"):
                yield Button("✅ Yes, I trust this workspace", id="btn-trust", variant="success")
...
class FolderPicker(ModalScreen):
    ...
    def compose(self) -> ComposeResult:
        with Vertical(id="picker-container"):
            yield Label("Select Folder (Press 'O' to Open, Esc to Cancel)", id="picker-title")
    ...
    def compose(self) -> ComposeResult:
        with Vertical(id="settings-container"):
            ...
            with Horizontal(classes="setting-row"):
                ...
                yield Switch(value=app.settings.get("auto_format", True), id="setting-auto_format")
                
            with Horizontal(classes="setting-row"):
                yield Label("Real-Time Python Linter", classes="setting-label")
                yield Switch(value=app.settings.get("linter", True), id="setting-linter")
                
            with Horizontal(classes="setting-row"):
                yield Label("Show Line Numbers", classes="setting-label")
                yield Switch(value=app.settings.get("line_numbers", True), id="setting-line_numbers")
                
            with Horizontal(classes="setting-row"):
                yield Label("Use EssaCache Hot-Loading", classes="setting-label")
                yield Switch(value=app.settings.get("cache_enabled", True), id="setting-cache_enabled")
...
class TerminalArea(Vertical):
    ...
    async def on_input_submitted(self, event: Input.Submitted) -> None:
        if event.input.id != "term-input": return
        cmd = event.value
        if not cmd: return
        event.input.value = ""
...
class EssaIDEApp(App):
    ...
    #sidebar-search-results, #sidebar-plugins-list {
        height: 1fr;
        background: #16161e;
    }
    ...
    def compose(self) -> ComposeResult:
        ...
        with Container(id="main-container"):
            ...
            with Vertical(id="sidebar"):
                with ContentSwitcher(initial="sidebar-explorer", id="sidebar-switcher"):
                    with Vertical(id="sidebar-explorer"):
                        yield Label(f"Explorer: {Path(self.workspace_path).name or '/'}", id="workspace-path", classes="sidebar-title")
                    ...
                    with Vertical(id="sidebar-search"):
                        ...
                        yield Input(placeholder="Search workspace...", id="sidebar-search-input")
                        yield Button("Find All", id="btn-sidebar-search", variant="primary")
            ...
            with Vertical(id="editor-container"):
                yield Input(placeholder="Search EssaSearch... (Press Enter to submit, Ctrl+F to close)", id="search-input")
                yield Input(placeholder="Enter new file name... (Press Enter to create, Ctrl+N to close)", id="new-file-input")
                with Horizontal(id="split-view"):
                    with Vertical(id="editor-left-wrap"):
                        with Horizontal(id="tab-bar-left"):
                            yield Button("✕", id="btn-close-left", classes="tab-close-btn")
                        with Vertical(id="welcome-screen"):
                            yield Static("\n\n  🚀  EssaIDE  ", id="welcome-logo")
                            yield Static("  The Professional Terminal IDE\n", id="welcome-sub")
                            yield Static("  Ctrl+O        Open Folder", classes="welcome-shortcut")
                            yield Static("  Ctrl+N        New File", classes="welcome-shortcut")
                            yield Static("  Ctrl+S        Save", classes="welcome-shortcut")
                            yield Static("  Ctrl+R        Run Code", classes="welcome-shortcut")
                            yield Static("  Ctrl+B        Toggle Split View", classes="welcome-shortcut")
                            yield Static("  Ctrl+Shift+P  Command Palette", classes="welcome-shortcut")
                        yield TabbedContent(id="editor-tabs")
                    yield VerticalResizer("editor-left-wrap", offset=38, id="split-resizer", classes="hidden")
                    with Vertical(id="editor-right-wrap", classes="hidden"):
                        with Horizontal(id="tab-bar-right"):
                            yield Button("✕", id="btn-close-right", classes="tab-close-btn")
                ...
                with TabbedContent(id="bottom-panel"):
                    ...
                    with TabPane("Console", id="tab-console"):
                        with Vertical(id="console-container"):
                            ...
                            yield Input(placeholder=">>> Type Python code...", id="console-input")
                    with TabPane("API Client", id="tab-api"):
                        with Vertical(id="api-container"):
                            yield Input(placeholder="Enter API URL (e.g., https://api.github.com) and press Enter...", id="api-input")
                            yield RichLog(id="api-log", highlight=True, markup=True)
                    with TabPane("Docker", id="tab-docker"):
                        with Vertical():
                            yield Button("Refresh Containers", id="btn-docker-refresh", variant="primary")
                            yield DataTable(id="docker-table")
                    with TabPane("Monitor", id="tab-monitor"):
                        with Vertical(id="monitor-container"):
                            yield Button("Refresh System Stats", id="btn-monitor-refresh", variant="success")
                    ...
                    with TabPane("Packages", id="tab-packages"):
                        with Vertical(id="pkg-container"):
                            with Horizontal(id="pkg-controls"):
                                yield Input(placeholder="Enter package name (e.g. requests)...", id="pkg-input")
                                yield Button("Install", id="btn-pkg-install", variant="success")
                                yield Button("List Installed", id="btn-pkg-list", variant="primary")
    ...
    def finish_initialization(self, pop_splash: bool = True) -> None:
        ...
        def _build_ast_index():
            ...
            for path in workspace.rglob("*.py"):
                ...
                try:
                    ...
                    continue
            self.call_from_thread(terminal.write, f"🧠 AST Indexer: Scanned {count} definitions across workspace.")
        ...
        if plugins_dir.exists() and plugins_dir.is_dir():
            if not self.workspace_trusted:
                terminal.write("🔒 [yellow]Plugins are DISABLED — workspace is not trusted.[/yellow]")
                terminal.write("    Click the 🧩 Extensions icon and trust the workspace to enable.")
            else:
                ...
                for plugin_file in plugins_dir.glob("*.py"):
                    ...
                    try:
                        ...
                        risky_modules = ["os", "subprocess", "socket", "shutil", "sys", "ctypes", "eval", "exec"]
                        for rm in risky_modules:
                            if f"import {rm}" in source or f"from {rm}" in source or f" {rm}." in source:
                                risks.append(rm)
                    except Exception:
                        pass
                    ...
                    manifest_entry = {"name": plugin_file.stem, "file": plugin_file.name, "risks": risks, "badge": badge, "loaded": False}
...
"""
                    try:
                        ...
                        output_lines = [l for l in result.stdout.strip().splitlines() if l.startswith("{")]
                        for line in output_lines:
                            try:
                                msg = _json.loads(line)
                                if msg.get("action") == "terminal":
                                    terminal.write(f"  [dim][plugin][/dim] {msg.get('msg', '')}")
                                elif msg.get("action") == "notify":
                                    self.notify(msg.get("msg", ""), title=plugin_file.stem)
                                elif msg.get("status") == "ok":
                                    terminal.write(f"  ✅ [bold cyan]{plugin_file.stem}[/bold cyan] — {badge}")
                                    manifest_entry["loaded"] = True
                                    loaded_count += 1
                                elif msg.get("status") == "no_setup":
                                    terminal.write(f"  ⚠️ {plugin_file.name} missing setup()")
                                elif msg.get("status") == "error":
                                    terminal.write(f"  ❌ {plugin_file.name}: {msg.get('msg', '')}")
                            except Exception:
                                pass
                    except subprocess.TimeoutExpired:
                        terminal.write(f"  ⏱️ {plugin_file.name} timed out (5s sandbox limit)")
                    except Exception as e:
                        terminal.write(f"  ❌ {plugin_file.name}: {e}")
                    self._plugin_manifest.append(manifest_entry)

                if loaded_count > 0:
                    terminal.write(f"✨ Sandboxed {loaded_count} extension(s) successfully.")
    ...
    def load_file(self, file_path: Path, split_id: str) -> None:
        ...
        open_tabs_map = self.open_tabs if split_id == "editor-tabs" else self.split_open_tabs
        ...
        try:
            ...
            if cached_content:
                terminal.write(f"⚡ CACHE HIT: Loaded {file_path.name} instantly from memory!")
            ...
            tab_id = f"{split_id}-{safe_name}-{len(open_tabs_map)}"
        ...
        for path, t_id in open_tabs_map.items():
            if t_id == pane_id:
                ...
                try:
                    ...
                    lang_map = {".py": "Python 🐍", ".md": "Markdown 📄", ".json": "JSON ⚙️", ".txt": "Text", ".js": "JS 🟨", ".html": "HTML 🌐", ".css": "CSS 🎨"}
    ...
    def on_tab_close_click(self, event: events.MouseDown) -> None:
        ...
        if tab_x >= widget.size.width - 4:
            ...
            if pane_id in self.split_open_tabs.values():
                path = next((k for k, v in self.split_open_tabs.items() if v == pane_id), None)
                if path:
                    self.split_open_tabs.pop(path)
                    if self.current_file_path == path: self.current_file_path = None
                    self.query_one("#editor-tabs-split", TabbedContent).remove_pane(pane_id)
            elif pane_id in self.open_tabs.values():
                path = next((k for k, v in self.open_tabs.items() if v == pane_id), None)
                if path:
    ...
    def on_selection_changed(self, event: TextArea.SelectionChanged) -> None:
        ...
        try:
            if not self.query("StatusBar"): return
            cursor = event.text_area.cursor_location
            self.query_one("#status-cursor", Label).update(f"Ln {cursor[0] + 1}, Col {cursor[1] + 1}")
        except Exception:
            pass

    def action_save(self) -> None:
        """Action for Ctrl+S."""
        open_tabs_map = self.open_tabs if self.active_split_id == "editor-tabs" else self.split_open_tabs
        if self.current_file_path and self.current_file_path in open_tabs_map:
            ...
            try:
                ...
                if self.current_file_path.suffix == ".py" and self.settings.get("auto_format", True):
                    import subprocess
                    try:
                        subprocess.run([sys.executable, "-m", "black", "-q", str(self.current_file_path)], check=True)
                        with open(self.current_file_path, "r", encoding="utf-8") as f:
                            formatted_text = f.read()
                        if editor.text != formatted_text:
                            editor.load_text(formatted_text)
                            self.query_one(TerminalArea).write("✨ Auto-formatted with Black")
                    except Exception:
                        pass
                ...
                if self.cache_client:
                    try:
                        ...
                        self.query_one(TerminalArea).write(f"🔄 Cache updated for {self.current_file_path.name}")
                    except Exception:
                        pass
                
                self.notify(f"Saved {self.current_file_path.name}", title="Success")
                self.query_one(TerminalArea).write(f"Saved: {self.current_file_path.name}")
            except Exception as e:
                self.notify(f"Error saving file: {e}", title="Error", severity="error")
                self.query_one(TerminalArea).write(f"Error saving {self.current_file_path.name}: {e}")
        else:
            self.notify("No file selected to save.", title="Warning", severity="warning")
    ...
    def action_toggle_split(self) -> None:
        ...
        if split.has_class("hidden"):
            ...
            self.active_split_id = "editor-tabs-split"
            self.query_one(TerminalArea).write("Split view opened. Focus moved to right pane.")
    ...
    def on_text_area_changed(self, event: TextArea.Changed) -> None:
        ...
        if not editor_id: return
        tab_id = editor_id.replace("editor-", "")
        split_id = "editor-tabs-split" if tab_id.startswith("editor-tabs-split") else "editor-tabs"
        ...
        if self.current_file_path and self.current_file_path.suffix == ".py" and self.settings.get("linter", True):
            ...
            try:
                ...
                problems.write(f"[red]Syntax Error in {self.current_file_path.name} line {e.lineno}: {e.msg}[/red]")
            except Exception:
                pass
    ...
    def action_delete_file(self) -> None:
        ...
        if self.current_file_path and self.current_file_path.exists():
            try:
                ...
                self.notify(f"Deleted {self.current_file_path.name}", title="Deleted", severity="warning")
                self.query_one(TerminalArea).write(f"🗑️ Deleted {self.current_file_path.name}")
                ...
                self.notify(f"Error deleting file: {e}", title="Error", severity="error")
        else:
            self.notify("No file selected to delete.", title="Warning", severity="warning")
    ...
    async def action_run_code(self) -> None:
        ...
        if ext in [".html", ".htm"]:
            terminal.write(f"🌐 Starting Live Server for {self.current_file_path.name}...")
            import subprocess
            import webbrowser
            try:
                subprocess.Popen([sys.executable, "-m", "http.server", "8080"], cwd=parent)
                webbrowser.open(f"http://localhost:8080/{self.current_file_path.name}")
                terminal.write("✅ Live Server started on port 8080. Check your browser!")
            except Exception as e:
        ...
        try:
            if ext in interpreters:
                ...
                terminal.write(f"🚀 Running {cmd[0].upper()}: {self.current_file_path.name}...")
                ...
                cmd = compilers[ext]
                terminal.write(f"⚙️ Compiling & Running: {self.current_file_path.name}...")
                process = await asyncio.create_subprocess_shell(
                    cmd,
                    stdout=asyncio.subprocess.PIPE,
                    stderr=asyncio.subprocess.PIPE,
                ...
                self.notify(f"Runner not supported for {ext} files.", title="Unsupported", severity="error")
                return
            ...
            async def read_stderr(stream):
                ...
                while True:
                    line = await stream.readline()
                    if not line: break
                    problems.write(f"[red]{line.decode('utf-8', errors='replace').rstrip()}[/red]")
                    self.query_one("#bottom-panel", TabbedContent).active = "tab-problems"
    ...
    def action_toggle_breakpoint(self) -> None:
        ...
        if self.current_file_path and self.current_file_path in open_tabs_map:
            if self.current_file_path.suffix == ".py":
                ...
                row_text = editor.get_text_range((cursor_loc[0], 0), (cursor_loc[0], cursor_loc[1]))
                indent = len(row_text) - len(row_text.lstrip())
                
                editor.insert(f"breakpoint()\n{' ' * indent}")
                self.notify("Breakpoint injected!", title="Debugger")
                self.query_one(TerminalArea).write("🔴 Breakpoint added. Use Ctrl+Y to run in Debug Mode.")
            else:
                self.notify("Breakpoints are only supported in Python.", severity="error")

    def action_run_debugger(self) -> None:
        """Suspend the app and run interactively to allow pdb."""
        if not self.current_file_path:
            self.notify("No file selected.", title="Warning", severity="warning")
            return
            
        if self.current_file_path.suffix != ".py":
            self.notify("Only Python files can be debugged.", title="Unsupported", severity="error")
            return
        ...
        with self.suspend():
            import os
            print(f"\n--- 🐛 Debugging {self.current_file_path.name} ---")
            print("If a breakpoint() is hit, you will drop into the pdb interactive shell.\n")
            os.system(f"{sys.executable} {self.current_file_path.absolute()}")
            print("\n--- Program Exited. Press Enter to return to EssaIDE ---")
            input()
            
        self.query_one(TerminalArea).write(f"✅ Returned from debugging {self.current_file_path.name}")
    ...
    async def action_profile_run(self) -> None:
        ...
        process = await asyncio.create_subprocess_exec(
            sys.executable, "-m", "cProfile", "-s", "tottime", str(self.current_file_path),
            stdout=asyncio.subprocess.PIPE,
            stderr=asyncio.subprocess.PIPE,
    ...
    def action_go_to_definition(self) -> None:
        ...
        if target_word in self.global_ast_index:
            ...
            terminal.write(f"🎯 AST Match: Jumping to definition in {path.name} (Line {line_idx+1})")
    ...
    def on_input_submitted(self, event: Input.Submitted) -> None:
        if event.input.id == "search-input":
            ...
            for path in workspace.rglob("*"):
                if path.is_file() and not path.name.startswith(".") and "__pycache__" not in str(path):
                    try:
                        with open(path, "r", encoding="utf-8") as f:
                            for i, line in enumerate(f):
                                if query in line:
                                    terminal.write(f"🎯 Found in {path.name} (Line {i+1})")
                                    self.load_file(path, self.active_split_id)
            ...
            try:
                ...
                self.notify(f"File {filename} already exists!", title="Error", severity="error")
            except Exception as e:
                self.notify(f"Failed to create file: {e}", title="Error", severity="error")
            ...
            def fetch_api():
                try:
                    ...
                    try:
                        ...
                        self.call_from_thread(api_log.write, f"[green]Status: {res.status_code}[/green]\n```json\n{formatted}\n```")
                    except Exception:
                        self.call_from_thread(api_log.write, f"[green]Status: {res.status_code}[/green]\n{res.text[:1000]}")
                except Exception as e:
    ...
    def refresh_docker(self, event: Button.Pressed) -> None:
        ...
        import subprocess
        try:
            out = subprocess.check_output(["docker", "ps", "--format", "{{.ID}}\t{{.Image}}\t{{.Status}}\t{{.Ports}}\t{{.Names}}"], text=True)
    ...
    def handle_auto_pairs(self, event: events.Key) -> None:
        ...
        if event.character == ">":
            ...
            void_elements = ["br", "img", "hr", "meta", "link", "input", "source", "area"]
        ...
        if event.key == "ctrl+t":
            ...
            if line.lstrip().startswith("#"):
                new_line = line.replace("# ", "", 1) if line.lstrip().startswith("# ") else line.replace("#", "", 1)
            else:
                indent = len(line) - len(line.lstrip())
    ...
    def execute_global_replace(self, target: str, replacement: str) -> None:
        import threading
        terminal = self.query_one(TerminalArea)
        terminal.write(f"🔄 Executing Global Refactor: '{target}' -> '{replacement}'...")
        ...
        def _replace():
            ...
            for path in workspace.rglob("*"):
                if path.is_file() and not path.name.startswith(".") and "__pycache__" not in str(path) and "node_modules" not in str(path) and ".git" not in str(path):
                    try:
                        with open(path, "r", encoding="utf-8") as f:
                            content = f.read()
            ...
            def _update_ui():
                terminal.write(f"✅ Refactoring Complete! Modified {lines_changed} instances across {files_changed} files.")
        ...
        def _install():
            try:
                ...
                if proc.returncode == 0:
                    self.call_from_thread(terminal.write, f"✅ Successfully installed {pkg_name}!")
                    self.call_from_thread(self._update_requirements, pkg_name)
                    self.call_from_thread(self.list_packages)
                else:
                    self.call_from_thread(terminal.write, f"❌ Failed to install {pkg_name}.")
            except Exception as e:
        ...
        try:
            if req_file.exists():
                ...
                with open(req_file, "w") as f:
                    f.write(f"{pkg_name}\n")
            self.query_one(TerminalArea).write(f"📝 Added {pkg_name} to requirements.txt")
        except Exception:
            pass
    ...
    def action_preview_markdown(self) -> None:
        ...
        if not self.current_file_path or self.current_file_path.suffix != ".md":
            self.notify("Only Markdown (.md) files can be previewed.", severity="warning")
            return
        ...
        if split_container.has_class("hidden"):
            ...
            self.query_one(TerminalArea).write("Split view opened for Markdown Preview.")
        ...
        try:
            ...
            self.query_one(TerminalArea).write(f"🔄 Refreshed Live Markdown Preview for {self.current_file_path.name}")
            return
        except Exception:
            pass
            
        md_viewer = Markdown(md_content, id=f"{preview_tab_id}-content")
        pane = TabPane(f"👀 {self.current_file_path.name}  ✕", md_viewer, id=preview_tab_id)
        split_tabs.add_pane(pane)
        split_tabs.active = preview_tab_id
        self.query_one(TerminalArea).write(f"📖 Live Markdown Preview launched for {self.current_file_path.name}")
    ...
    def action_start_live_server(self) -> None:
        ...
        if self.live_server_process is not None:
            ...
            try:
                ...
                term.write("[bold green]🌐 Live Server started at http://localhost:8000[/bold green]")
    ...
    def show_plugins(self):
        ...
        if self._plugin_manifest:
            ...
            for entry in self._plugin_manifest:
                ...
                lst.add_option(f"🧩 {entry.get('name')} {loaded} (v{entry.get('version')})")
        else:
            from pathlib import Path
            plugins_dir = Path(self.workspace_path) / ".essa_plugins"
    ...
    def on_plugin_selected(self, event: OptionList.OptionSelected) -> None:
        ...
        if "🧩" in prompt_text:
            ...
            self.query_one(TerminalArea).write(f"[bold cyan]Extension {ext_name} activated.[/bold cyan]")
    ...
    def do_sidebar_search(self):
        ...
        def _search():
            try:
                out = subprocess.check_output(["grep", "-rn", query, self.workspace_path], text=True)
